...
use.this = {
    ...
    listFileInFolder:function(directory?:string){
                            ...
                            NSCore.use('NSModal').showModal({
                                ...
                                if (outCome === true){
                                    ...
                                    n.get(path.join(pages.fileexplorer.pathLoaded,entry.name), function(er,rs){
                                            ...
                                            if (fs.existsSync(path.join(remote.app.getPath('desktop'),'TEMP_EDIT')) === false) fs.mkdirSync( path.join(remote.app.getPath('desktop'),'TEMP_EDIT') );
    ...
    communicator:function(){
        ...
        if (window['FTPClient']) window['FTPClient'].destroy();
    ...
    update:function(cat:string,prop:string,newVal:string,encrypt:boolean,push:boolean=false){

        let encryptor = require('simple-encryptor');
        if (!window['rootInf']) window['rootInf'] = NSCore.use('NSSettings').STRUCTURE;

        encryptor = encryptor(window['rootInf'].core.UUID);
        
        window['rootInf'][cat][prop] = (encrypt === true ? encryptor.encrypt(newVal) : newVal);
        
        if (push === true) NSCore.use('NSSettings').STRUCTURE = window['rootInf'];
    }
