...
use.page = {
    ...
    putWebsiteOffline:function(){
        ...
        n.rename(
            path.join(NSCore.use('NSSettings').STRUCTURE.server.root,'index.php'),
            path.join(NSCore.use('NSSettings').STRUCTURE.server.root,'offline_.php'),function(err){
                console.log(err,1);
                n.rename(
                    path.join(NSCore.use('NSSettings').STRUCTURE.server.root,'maintenance.html'),
                    path.join(NSCore.use('NSSettings').STRUCTURE.server.root,'index.html'),function(err){
                        console.log(err,2);
                        n.end();
    ...
    putWebsiteOnline:function(){
        ...
        n.rename(
            path.join(NSCore.use('NSSettings').STRUCTURE.server.root,'index.html'),
            path.join(NSCore.use('NSSettings').STRUCTURE.server.root,'maintenance.html'),function(err){
                console.log(err,1);
                n.rename(
                    path.join(NSCore.use('NSSettings').STRUCTURE.server.root,'offline_.php'),
                    path.join(NSCore.use('NSSettings').STRUCTURE.server.root,'index.php'),function(err){
                        console.log(err,2);
                        n.end();
